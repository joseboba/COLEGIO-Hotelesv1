...
function login(req, res){
    ...
    if((params.mail || params.username) && params.password){
        User.findOne({$or:[{username: params.username}, {mail: params.mail}]}, (err, foundUser) =>{
            if(err){
                res.status(500).send(err);
            }else if(foundUser){
                bcrypt.compare(params.password, foundUser.password, (err, password) =>{
                    if(err){
                        res.status(500).send(err);
                    }else if(password){
                ...
                res.status(404).send({message: 'Usuario o contraseña incorrectos', foundUser});
            }
        });
    }else{
                         ...
                         if(err){
                            ...
                            user.save((err, userSaved) =>{
                                if(err){
                                    ...
                                    res.send({message: 'El siguiente usuario se ha guardado correctamente', userSaved});
                                }else{
        ...
        if(id != req.user.sub){
            ...
            User.findOne({$or:[{username: params.username}, {mail: params.mail}]}, (err, foundUser) =>{
                if(err){
                    ...
                    res.status(406).send({message: 'No se puede asignar usuarios o correos que ya existen'});
                }else{
                    if(params.password){
                        bcrypt.hash(params.password, null, null, (err, password) =>{
                            if(err){
                                ...
                                User.findByIdAndUpdate(id, params, {new: true}, (err, updated) =>{
                                    if(err){
                                        ...
                                        res.send({message: 'Se ha actualizado con éxtio el siguiente usuario: ', updated});
                                    }else{
                                        res.status(418).send({message: 'No se pudo actualizar el usuario'});
                                    }
                                });
                            }else{
                                res.status(418).send({message: 'No se pudo encriptar la contraseña'});
                            }
                        });
                    }else{
                        User.findByIdAndUpdate(id, params, {new: true}, (err, updated) =>{
...
function range(req, res){
    ...
    if(id != req.user.sub){
        ...
        if(params.dates){
            Hotel.find({dates:{$lte:new Date(params.dates)}},null,{sort:{dates:-1}}, (err, found) =>{
                if(err){
                    res.status(500).send(err);
                }else if(found){
                    res.send(found);
